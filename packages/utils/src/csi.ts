/**
 * CSI MasterFormat Division reference data.
 * Full 50-division list per CSI MasterFormat 2016.
 */
export interface CsiDivisionData {
  code: string;
  name: string;
  description?: string;
}

export const CSI_DIVISIONS: CsiDivisionData[] = [
...
/**
 * Look up a CSI division by code.
 */
export function getCsiDivision(code: string): CsiDivisionData | undefined {
 ...
 */
export function getCsiDivisionName(code: string): string {
  const division = getCsiDivision(code);
  return division ? division.name : `Division ${code}`;
}

/**
 * Format a CSI division for display.
 * Example: "03" → "Division 03 — Concrete"
 */
export function formatCsiDivision(code: string): string {
  const division = getCsiDivision(code);
  if (!division) return `Division ${code}`;
  return `Division ${code} — ${division.name}`;
}

/**
 * Search CSI divisions by name or code.
 */
export function searchCsiDivisions(query: string): CsiDivisionData[] {
  const lower = query.toLowerCase();
  return CSI_DIVISIONS.filter(
    (d) =>
